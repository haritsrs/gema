...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const recentPostsQuery = query(postsRef, orderByChild('createdAt'), limitToLast(7));

    const unsubscribe = onValue(recentPostsQuery, (snapshot) => {
      if (snapshot.exists() && !isRefreshing) {
        const postsData = [];
  ...
  const fetchOlderPosts = async (lastTimestamp = null) => {
  ...
  const handleObserver = (entries) => {
    const [entry] = entries;
    if (entry.isIntersecting && !loading && lastVisibleTimestamp && !noMorePosts) {
      fetchOlderPosts(lastVisibleTimestamp);
    }
  };
  ...
  }, [lastVisibleTimestamp, loading, noMorePosts]);
    ...
    <div className={`${geistSans.variable} ${geistMono.variable} antialiased min-h-screen w-full bg-gray-950`}>
      <div className="max-w-2xl mx-auto px-4">
        <div className="space-y-4 py-4">
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg bg-purple-600 text-white ${
                isRefreshing ? 'opacity-50 cursor-not-allowed' : 'hover:bg-purple-700'
              }`}
            >
          ...
          <Posting onPostCreated={handleRefresh} storage={storage} />
